...
// 更新行
const updateCompetitionInfoFunc = async (row) => {
  const res = await findCompetitionInfo({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteCompetitionInfoFunc = async (row) => {
  const res = await deleteCompetitionInfo({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
