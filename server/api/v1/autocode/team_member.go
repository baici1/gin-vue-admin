...
func (teamMemberApi *TeamMemberApi) CreateOwnTeamMember(c *gin.Context) {
	var param autocodeReq.OwnTeamMember
	if err := c.ShouldBind(&param); err != nil {
		response.ValidatorError(err, c)
		return
	}
	if err := teamMemberService.CreateOwnTeamMember(param); err != nil {
		global.GVA_LOG.Error("增加失败!", zap.Error(err))
