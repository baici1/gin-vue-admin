...
func CreateConsumer() (*consumer, error) {
	...
	durable := global.GVA_CONFIG.Rabbitmq.HelloWorld.Durable
	chanNumber := global.GVA_CONFIG.Rabbitmq.HelloWorld.ConsumerChanNumber
	reconnectInterval := global.GVA_CONFIG.Rabbitmq.HelloWorld.OffLineReconnectIntervalSec
	retryTimes := global.GVA_CONFIG.Rabbitmq.HelloWorld.RetryCount
...
type consumer struct {
	...
	offLineReconnectIntervalSec int
	retryTimes                  int
	...
	go func() {
		...
		case err := <-c.connErr:
			...
			for i = 1; i <= c.retryTimes; i++ {
				// 自动重连机制
				time.Sleep(time.Duration(c.offLineReconnectIntervalSec) * time.Second)
