...
func CreateConsumer(options ...OptionsConsumer) (*consumer, error) {
	// 获取配置信息
	conn, err := amqp.Dial(global.GVA_CONFIG.Rabbitmq.Routing.Addr)
	queueName := global.GVA_CONFIG.Rabbitmq.Routing.QueueName
	durable := global.GVA_CONFIG.Rabbitmq.Routing.Durable
	exchangeType := global.GVA_CONFIG.Rabbitmq.Routing.ExchangeType
	exchangeName := global.GVA_CONFIG.Rabbitmq.Routing.ExchangeName
	reconnectInterval := time.Duration(global.GVA_CONFIG.Rabbitmq.Routing.OffLineReconnectIntervalSec)
	retryTimes := global.GVA_CONFIG.Rabbitmq.Routing.RetryCount
